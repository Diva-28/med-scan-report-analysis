"""
Ultra-fast Medical Scan Report Chatbot - Demo version with mock analysis.
Upload scan → Get QR code + PDFs in < 5 seconds.
"""
import gradio as gr
import io
import time
from pathlib import Path
from PIL import Image
from generate_scan_report_pdf import generate_report
from generate_prescription_pdf import generate_prescription
import base64
import qrcode
from datetime import datetime
import uuid
import json


def generate_qr_instant(token: str):
    """Generate QR code instantly (no server call)."""
    verification_url = f"http://localhost:8000/verify?token={token}"
    
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECT_L,
        box_size=10,
        border=2,
    )
    qr.add_data(verification_url)
    qr.make(fit=True)
    
    img = qr.make_image(fill_color="black", back_color="white")
    # Convert to PIL Image (Gradio compatible format)
    pil_img = Image.new('RGB', img.size, 'white')
    pil_img.paste(img)
    return pil_img


def generate_mock_analysis(file_path: str) -> dict:
    """
    Generate mock brain scan analysis for demo.
    In production, replace with real ML model inference.
    """
    try:
        # Verify file exists and is an image
        img = Image.open(file_path)
        
        # Mock analysis data (simulates ML model output)
        analysis_options = [
            {
                "has_tumor": True,
                "tumor_type": "Glioma",
                "size": "1.2 cm",
                "location": "Right occipital lobe",
                "confidence": 0.92,
                "analysis_details": {
                    "anomalous_regions": 2,
                    "edge_complexity": 151,
                    "tumor_score": 0.85,
                    "region_intensities": [220.39, 158.87, 210.12, 176.93, 134.04, 160.1, 176.74, 124.37]
                }
            },
            {
                "has_tumor": True,
                "tumor_type": "Meningioma",
                "size": "0.8 cm",
                "location": "Left temporal lobe",
                "confidence": 0.88,
                "analysis_details": {
                    "anomalous_regions": 1,
                    "edge_complexity": 98,
                    "tumor_score": 0.72,
                    "region_intensities": [210.2, 145.5, 198.1, 165.3, 125.8, 152.9, 168.5, 118.2]
                }
            },
            {
                "has_tumor": False,
                "tumor_type": None,
                "size": None,
                "location": None,
                "confidence": 0.95,
                "analysis_details": {
                    "anomalous_regions": 0,
                    "edge_complexity": 45,
                    "tumor_score": 0.12,
                    "region_intensities": [180.5, 175.2, 178.9, 172.1, 176.3, 174.8, 179.2, 173.5]
                }
            }
        ]
        
        # Use file hash to select deterministic option
        import random
        file_hash = hash(Path(file_path).read_bytes())
        random.seed(file_hash)
        return random.choice(analysis_options)
        
    except Exception as e:
        return {
            "error": str(e),
            "has_tumor": False,
            "tumor_type": None,
            "size": None,
            "location": None,
            "confidence": 0.5,
            "analysis_details": {
                "anomalous_regions": 0,
                "edge_complexity": 0,
                "tumor_score": 0.0,
                "region_intensities": []
            }
        }


def ultra_fast_analyze_and_report(file):
    """
    ULTRA-FAST FLOW (< 5 seconds):
    1. Upload scan
    2. Mock analysis (instant)
    3. Generate QR instantly
    4. Generate PDFs instantly
    5. Return all outputs
    """
    start_time = time.time()
    
    if not file:
        return None, None, None, "❌ No file provided"
    
    try:
        # Get file path
        file_path = file.name if hasattr(file, 'name') else str(file)
        
        # Step 1: Analyze scan (using mock for instant results)
        analysis = generate_mock_analysis(file_path)
        
        if "error" in analysis:
            return None, None, None, f"❌ File processing failed: {analysis['error']}"
        
        # Step 2: Generate unique token for QR
        token = str(uuid.uuid4())[:16]
        
        # Step 3: Generate QR code instantly
        try:
            qr_img = generate_qr_instant(token)
        except Exception as e:
            qr_img = None
            print(f"Warning: QR generation failed: {e}")
        
        # Step 4: Generate PDFs instantly
        try:
            out_dir = str(Path(__file__).parent)
            report_path = generate_report(analysis, out_dir=out_dir)
            prescription_path = generate_prescription(analysis, patient_id=token[:8].upper(), out_dir=out_dir)
        except Exception as e:
            return None, None, qr_img, f"❌ PDF generation failed: {str(e)}"
        
        elapsed = time.time() - start_time
        
        # Format confidence percentage
        confidence_val = analysis.get('confidence', 0)
        if isinstance(confidence_val, (int, float)):
            confidence_pct = f"{round(confidence_val * 100, 1)}%"
        else:
            confidence_pct = "N/A"
        
        success_msg = (
            f"✅ **Report Generated in {elapsed:.2f}s**\n\n"
            f"📄 Scan Report: {Path(report_path).name}\n"
            f"💊 Prescription: {Path(prescription_path).name}\n"
            f"🔐 Token: {token}\n\n"
            f"**Analysis Results:**\n"
            f"• Tumor Detected: {'Yes' if analysis.get('has_tumor') else 'No'}\n"
            f"• Tumor Type: {analysis.get('tumor_type', 'N/A')}\n"
            f"• Size: {analysis.get('size', 'N/A')}\n"
            f"• Location: {analysis.get('location', 'N/A')}\n"
            f"• Confidence: {confidence_pct}"
        )
        
        return report_path, prescription_path, qr_img, success_msg
        
    except Exception as e:
        print(f"Error in ultra_fast_analyze_and_report: {e}")
        import traceback
        traceback.print_exc()
        return None, None, None, f"❌ Error: {str(e)}"


# Create the ultra-fast Gradio interface
demo = gr.Interface(
    fn=ultra_fast_analyze_and_report,
    inputs=gr.File(label="📤 Upload Brain Scan Image", file_types=["image"]),
    outputs=[
        gr.File(label="📄 Download Scan Report"),
        gr.File(label="💊 Download Prescription"),
        gr.Image(label="🔐 QR Code"),
        gr.Textbox(label="📋 Status & Results", lines=6),
    ],
    title="⚡ Ultra-Fast AI Brain Scan Report",
    description="Upload a brain scan → Get instant report, prescription & QR (< 5 seconds)",
    theme=gr.themes.Soft(),
)


if __name__ == "__main__":
    import os
    port = int(os.environ.get("GRADIO_PORT", "7864"))
    print(f"\n🚀 Ultra-Fast Chatbot on http://127.0.0.1:{port}")
    print(f"📊 Response time: < 5 seconds\n")
    demo.launch(share=False, server_name="127.0.0.1", server_port=port)
